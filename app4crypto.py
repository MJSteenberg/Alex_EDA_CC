...
st.title('Crypto Price App by Blitz')
st.markdown("""
...
* **Credit:** Web scraper adapted from the Medium article *[Web Scraping Crypto Prices With Python](https://towardsdatascience.com/web-scraping-crypto-prices-with-python-41072ea5b5bf)*. App written by Blitz.
""")
...
currency_price_unit = col1.selectbox('Select currency for price', ('USD', 'ZAR'))

# Web scraping of CoinMarketCap data
@st.cache
def load_data():
    c = CurrencyRates()
    convert_rand = c.get_rate('USD', 'ZAR')
    ...
    if currency_price_unit == 'ZAR':
        df['price'] = df['price'] * int(convert_rand)
        df.rename(columns={'price':'price_zar'}, inplace=True)
    else:
        df['price'] = price
    return df
